...
All quantitative discussion is based on archived online results under \texttt{Results/} for seven selected KITTI Tracking sequences: 0003, 0006, 0007, 0009, 0010, 0013, and 0020. These sequences were prioritized because they contain sufficiently visible and persistent dynamic vehicles, making them well suited for exposing the benefit of localization-tracking feedback; by contrast, the other candidate datasets or sequences currently available to this project contain fewer dynamic objects and are therefore less informative for stress-testing a dynamic-scene loop. The report emphasizes both algorithmic intent and engineering realization, including ROS interfaces, time synchronization, pose buffering, standardized result archiving, and failure-mode analysis. On this selected seven-sequence online slice, the front-end configuration \texttt{FAST-LIO+MCTrack} improves ATE RMSE over the pure FAST-LIO baseline in every case, showing that the most reliable gain comes from the front-end closed loop of pose-assisted tracking and tracking-informed dynamic-point down-weighting. The joint backend can help on some drift-dominated sequences, but it is less stable and can fail when velocity extrapolation, timestamp alignment, or object gating are unreliable; Sequence 0009 is the clearest example. The overall conclusion is intentionally modest: this project is not a state-of-the-art joint SLAMMOT system, but a repository-grounded engineering prototype whose results can be rerun, inspected, and audited within the current project workflow. It validates the feasibility of a localization-tracking feedback loop and clarifies the transition toward future object-centric joint optimization. The full code, scripts, and experiment structure are available at \url{https://github.com/XC-CN/ME5400}.
\end{abstract}
...
\begin{itemize}[leftmargin=2em]
    ...
    \item A script-driven experiment framework with one-click baseline versus optimized runs inside the current project workflow, standardized archiving under \texttt{Results/}, TUM-format trajectory recording, and unified quantitative evaluation.
...
The main experimental conclusion of the report is therefore unambiguous at the level actually tested here. Within the seven selected online sequences and the current archived workflow, the project validates a useful front-end closed loop between localization and tracking whose gains are consistently observable and traceable. That loop is the most stable source of gain in the current system. The backend direction is promising but still immature, and its failures are informative rather than accidental. For a course project, this is a strong result: the system does not merely show that some improvement exists, but identifies where the improvement is robust, where it is fragile, and why the difference occurs.

\section{Conclusion and Future Plan}
This report expands the ME5400 project into a fuller technical narrative while keeping the conclusions aligned with the actual repository state. The project already achieves something meaningful: it builds a complete bidirectional loop among FAST-LIO, PointPillars, and MCTrack; it exposes that loop through explicit ROS topics and project-defined messages; it provides scripted, rerunnable experiments within the current repository workflow; and it evaluates the resulting trajectories quantitatively across seven selected KITTI Tracking sequences. In that sense, the work is already more than a simple component survey or a one-off demonstration. It is a functioning engineering validation platform for studying how localization and tracking can help one another in dynamic driving scenes.
...
\bibitem{mctrack}
X. Wang, S. Qi, J. Zhao, H. Zhou, S. Zhang, G. Wang, K. Tu, S. Guo, J. Zhao, J. Li, \emph{et al.}, ``MCTrack: A unified 3D multi-object tracking framework for autonomous driving,'' \emph{arXiv preprint arXiv:2409.16149}, 2024.
