...
Localization and perception in dynamic road scenes impose a contradiction that is easy to state but difficult to resolve in a full system. LiDAR SLAM and LiDAR-inertial odometry are most reliable when the majority of observed geometry is static. By contrast, autonomous driving perception cares most about the exact objects that violate this assumption: surrounding vehicles, pedestrians, cyclists, and other traffic participants. This conflict causes map contamination, registration degradation, and drift accumulation in traditional SLAM systems. Meanwhile, Multi-Object Tracking (MOT) presents an opposite necessity: maintaining coherent track topologies and accurate motion states heavily depends on robust ego-motion compensation. Therefore, the relationship is fundamentally bidirectional: better localization improves tracking, while tracked dynamic contexts could supply essential feedback to secure localization.

This project is built around that bidirectional intuition. The system integrates FAST-LIO~\cite{fastlio2} as the localization backbone, PointPillars~\cite{pointpillars} for 3D detection, and MCTrack~\cite{mctrack} as the online tracker. Tracked objects are then utilized in two ways: first, they are converted into soft dynamic weights to mitigate the misleading effects of dynamic points inside FAST-LIO; second, they are incorporated into an ego-only joint backend to test whether object observations can supply additional sliding-window constraints. To balance the course schedule and engineering robustness, soft down-weighting and a loosely-coupled message loop are adopted over a tightly-coupled object-centric graph.

The project pursues four coupled objectives: the \textbf{research objective} tests the bidirectional feedback between localization and tracking; the \textbf{engineering objective} builds a repeatable and modular ROS pipeline; the \textbf{evaluation objective} identifies stable sources of improvement in the loop; and the \textbf{diagnostic objective} exposes the failure mechanisms surrounding the joint backend.

In more concrete terms, the contributions of the current report and implementation can be summarized as follows.
\begin{itemize}[leftmargin=2em]
    \item A complete ROS-based integration of FAST-LIO, PointPillars, MCTrack, and an ego-only backend, with project-defined messages.
    ...
    \item A script-driven experiment framework supporting one-click reproducible runs, standardized archiving under \texttt{Results/}, and unified quantitative evaluation.
    \item A failure-aware analysis showing that the front-end loop is a stable source of improvement, whereas the current backend remains fragile under velocity extrapolation and timing uncertainty.
\end{itemize}

The choice of KITTI Tracking as the main validation dataset is deliberate, prioritizing sequences with sufficiently strong dynamic-vehicle activity (e.g., 0003, 0006, 0007, 0009, 0010, 0013, and 0020) to make the effects of tracking feedback visible~\cite{kitti}. \textbf{This report is not written as a claim of state-of-the-art performance.} Rather, it documents an engineering prototype that validates the feasibility of a bidirectional localization-tracking loop, demonstrates a full experiment pipeline, and makes its failure cases transparent, laying a solid foundation for future research.
