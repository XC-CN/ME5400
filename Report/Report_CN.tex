...
\begin{table}[H]
    ...
    \footnotesize
    \begin{tabularx}{\textwidth}{p{2.0cm} p{2.5cm} p{2.8cm} p{2.0cm} X}
        \toprule
        工作 & 传感器 / 前端 & 动态处理方式 & 实时性 / 复杂度 & 与本文关系 \\
        \midrule
        StaticFusion & RGB-D；稠密直接法 & 联合分割与背景融合 & 面向实时稠密建图~\cite{staticfusion} & 说明保守保留背景结构和加权处理可以直接改善定位。 \\
        DS-SLAM & 视觉；ORB-SLAM2 + 语义线程 & 语义过滤 + 运动一致性 & 多线程实时，工程友好~\cite{ds-slam} & 说明松耦合工程系统也能提高动态场景定位鲁棒性。 \\
        DynaSLAM & 视觉；ORB-SLAM2 家族 & 动态检测 + 背景恢复 & 较慢但精度高~\cite{dynaslam} & 代表把动态物体主要视为定位干扰源的强基线。 \\
        Conf SLAMMOT & 激光；激光里程计 & 置信度引导关联与对象因子 & 紧耦合，高复杂度~\cite{confslammot} & 启发对象置信度不应只用于筛选，也应影响定位约束强度。 \\
        LIMOT & 激光+惯性；LIO & 实时联合 LIO 与 MOT & 实时，但实现复杂~\cite{limot} & 是本文长期目标最接近的紧耦合参考。 \\
        IMM-SLAMMOT & 双目视觉；ORB-SLAM2 & 多运动模型切换 & 建模更强，代价更高~\cite{imm-slammot} & 说明单一匀速模型不足以稳定支撑动态场景定位。 \\
        Online Dynamic SLAM & 双目视觉；自研 VO & 基于 iSAM2 的动态增量平滑 & 更可扩展的在线后端~\cite{online-dynamic-slam} & 指出动态对象一旦进入估计问题，增量式后端就变得关键。 \\
        DL-SLOT & 激光；激光里程计 & 定位-跟踪协同图优化 & 紧耦合，优化层共享信息~\cite{dl-slot} & 说明对象级约束可以在优化层直接服务定位。 \\
        \bottomrule
    \end{tabularx}
\end{table}
