...
\author{ME5400 项目组\\Zhang Zhengyang (A0318487N)\\Wu Zining (A0294373W)}
\date{\today}
...
所有定量讨论均基于 \texttt{Results/} 目录下 7 个 KITTI 跟踪序列（0003, 0006, 0007, 0009, 0010, 0013, 0020）的归档在线实验结果。之所以优先选用这些序列，是因为它们包含更明显、更持续的动态车辆目标，能够充分暴露定位与跟踪之间的反馈收益；相比之下，本项目当前可直接对比的其他候选数据集或序列中的动态物体较少，不足以同等强度地检验动态场景闭环。报告强调算法设计意图与工程实现细节，包括 ROS 接口、时间同步、位姿缓冲、标准结果归档以及失效模式分析。在所有 7 个序列中，前端配置 \texttt{FAST-LIO+MCTrack} 在 ATE RMSE 指标上均始终优于纯 FAST-LIO 基线，表明最可靠的增益源自“位姿辅助跟踪”与“跟踪反馈式动态降权”形成的前端闭环。联合后端虽在部分漂移严重的序列上有所帮助，但其稳定性较弱，在速度外推、时间戳对齐或目标门控不可靠时可能失效，序列 0009 便是最典型的例子。本报告的总体结论采用了审慎的态度：本项目并非追求 SOTA 的联合 SLAMMOT 系统，而是一个可复现的工程原型，它验证了定位-跟踪反馈闭环的可行性，并为未来向以对象为中心（object-centric）的联合优化转型奠定了基础。完整代码、脚本与结果归档入口公开于 \url{https://github.com/XC-CN/ME5400}。
\end{abstract}
...
本报告中的实验统一建立在已经归档于 \texttt{Results/} 目录下的在线结果之上。分析的在线序列包括 0003、0006、0007、0009、0010、0013 和 0020。所有数值均直接取自各自的 \texttt{metrics.json} 文件，而文中讨论的图像则对应 \texttt{evaluation\_result.png}、\texttt{evaluation\_joint\_backend.png} 和 \texttt{ablation\_bar.png}。因此，本节并不引入在写报告期间新跑出来的额外实验，而是对已经通过可复现实验管线生成的结果进行归纳和解释。
...
\begin{figure}[H]
    ...
    \caption{KITTI 数据采集平台。本报告中的所有实验都基于已归档的 KITTI Tracking 结果。}
    \label{fig:kitti_vehicle}
\end{figure}

\subsection{在线实验：汇总结果}
表~\ref{tab:online_ate} 汇总了全部在线序列的 ATE 和 RPE。最重要的整体观察非常直接：\texttt{FAST-LIO+MCTrack} 在全部 7 个在线序列上都改善了 ATE RMSE。这种 7/7 的表现，比任何单个“大成功案例”都更有说服力，因为它表明前端闭环具有跨场景稳定性。相比之下，\texttt{Joint Backend} 只在 4 个序列上进一步降低了 ATE，其中还有两个改善几乎可以视为持平。因此，后端并没有表现出与前端同等级别的跨序列稳定性。
...
\subsection{从现象到解释}
将上述分组结果综合起来，可以得到一个清晰的因果解释。当基线主要受到动态目标污染前端匹配的影响时，当前系统会稳定获益，因为动态降权正是在配准和建图误差积累之前打断了这一机制。这就是为什么前端闭环改善了全部 7 个序列，并且在高漂移动态场景中收益最大。相反，当基线已经相对稳定时，后端的剩余改善空间变小，而通过被跟踪对象施加的额外约束却更容易把噪声重新注入自车位姿。
...
因此，本报告的核心实验结论是明确的。当前项目已经验证了一个有用而且可复现的前端定位-跟踪闭环，而这个闭环正是整个系统中最稳定的收益来源。后端方向是有前景的，但仍然不成熟；它的失败并非偶然噪声，而是对下一阶段设计有诊断价值的证据。对于课程项目而言，这已经是一个很强的结果：系统不仅展示了“确实存在改善”，还清楚地区分了改善在哪里是稳健的，在哪里是脆弱的，以及这种差异为什么会出现。

\section{结论与未来计划}
本报告在保持结论与仓库实际状态一致的前提下，把 ME5400 项目扩展成了一套更完整的技术叙述。这个项目已经完成了一件真正有意义的事：它建立了 FAST-LIO、PointPillars 和 MCTrack 之间的完整双向闭环；它通过明确的 ROS 话题和项目定义消息将这个闭环暴露出来；它提供了一键可复现的实验流程；并且对 7 个 KITTI Tracking 在线序列上的轨迹结果进行了定量评估。从这个意义上说，它已经不再只是一个组件综述或一次性演示，而是一个真正运作的工程验证平台，用于研究动态驾驶场景下定位与跟踪如何相互增益。
...
因此，未来工作的优先级也很清晰。\textbf{第一}，后端应从仅自车修正器演化为一个\textbf{对象中心化联合后端}，在其中对象状态与自车位姿一起被优化，而不是只作为外部观测输入。这是最重要的升级，因为它直接回应了当前系统无法在优化内部吸收对象状态误差的问题。\textbf{第二}，系统需要\textbf{更强的置信度建模与关联策略}。检测得分和轨迹长度是有用的第一步启发，但不足以决定对象因子何时应当强烈影响自车运动。面向置信度的加权、更显式的运动一致性检查以及更丰富的数据关联诊断都仍然需要。\textbf{第三}，优化过程应朝着\textbf{增量式或更高效的滑窗更新}演化。如果对象中心化形式变得更丰富，当前这种轻量级最小二乘循环就不再足够。\textbf{第四}，项目需要在\textbf{更多数据集和更完整的消融实验}上继续验证，不是为了机械地增加篇幅，而是为了检验当前“前端稳定、后端脆弱”的模式是否会在 KITTI 之外依旧成立。

从课程项目价值的角度看，这是一个强且诚实的结果。该项目已经把多个复杂模块集成进统一工作流，证明了定位-跟踪反馈能够稳定地产生前端收益，也明确暴露了通往更强联合后端所面临的真实工程缺口。一个能工作的系统、可复现的实验，以及透明的失败分析，正是让当前原型对后续研究依然有价值的原因。如果下一轮迭代能够把今天这种消息级反馈提升为真正的对象中心化估计图，那么这个项目就能自然地从课程规模的工程验证，继续演化为动态场景 LiDAR-惯导 SLAMMOT 方向上更实质性的研究贡献。
