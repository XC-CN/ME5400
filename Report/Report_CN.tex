...
所有定量讨论均基于 \texttt{Results/} 目录下 7 个所选 KITTI 跟踪序列（0003, 0006, 0007, 0009, 0010, 0013, 0020）的归档在线实验结果。之所以优先选用这些序列，是因为它们包含更明显、更持续的动态车辆目标，能够充分暴露定位与跟踪之间的反馈收益；相比之下，本项目当前可直接对比的其他候选数据集或序列中的动态物体较少，不足以同等强度地检验动态场景闭环。报告强调算法设计意图与工程实现细节，包括 ROS 接口、时间同步、位姿缓冲、标准结果归档以及失效模式分析。在这 7 个所选在线序列上，前端配置 \texttt{FAST-LIO+MCTrack} 在 ATE RMSE 指标上均优于纯 FAST-LIO 基线，表明最可靠的增益源自“位姿辅助跟踪”与“跟踪反馈式动态降权”形成的前端闭环。联合后端虽在部分漂移严重的序列上有所帮助，但其稳定性较弱，在速度外推、时间戳对齐或目标门控不可靠时可能失效，序列 0009 便是最典型的例子。本报告的总体结论采用了审慎的态度：本项目并非追求 SOTA 的联合 SLAMMOT 系统，而是一个立足仓库现状的工程原型；在当前项目工作流内，其结果可以被重新运行、检查并追溯。它验证了定位-跟踪反馈闭环的可行性，并为未来向以对象为中心（object-centric）的联合优化转型奠定了基础。完整代码、脚本与结果归档入口公开于 \url{https://github.com/XC-CN/ME5400}。
\end{abstract}
...
据此，本项目追求四个关联的目标。\textbf{研究目标}是测试定位与跟踪在不构建完整统一优化器的情况下，能否在动态驾驶场景中形成有意义的反馈闭环。\textbf{工程目标}是构建一个可运行的 ROS 管线，使其组件、消息契约、启动顺序、结果日志及评估脚本在当前项目工作流内都是显式且可重复运行的。\textbf{评估目标}是确定闭环中哪一部分提供了最稳定的收益：是前端权重环节、后端修正环节，还是两者兼有。\textbf{诊断目标}是识别阻碍当前系统表现得像一个鲁棒的联合 SLAMMOT 方案的具体失效模式。
...
\begin{itemize}[leftmargin=2em]
    ...
    \item 建立了脚本驱动的实验框架，支持在当前项目工作流内进行基线与优化运行的一键切换、\texttt{Results/} 下的标准归档、TUM 格式轨迹记录及统一的定量评估。
    \item 通过失效感知分析表明，前端闭环是最稳定的改进源，而当前的后端在速度外推及时间不确定性面前仍显脆弱。
\end{itemize}

选择 KITTI Tracking 作为主要验证数据集是深思熟虑的结果。KITTI 为真实的城市驾驶场景提供了同步的激光雷达、IMU、标定信息及自车运动真值~\cite{kitti}。其 Tracking 切片包含测试定位-跟踪闭环所需的动态车辆、轨迹变化及背景演变。本文使用的 7 个在线序列（0003, 0006, 0007, 0009, 0010, 0013, 0020）涵盖了不同的漂移水平、动态强度及相对轨迹跨度，更重要的是，这些序列都存在足够明显的动态车辆参与者，能够让动态点抑制、跟踪反馈和后端对象因子的效果真正显现出来。相比之下，本项目当前其余可直接利用的候选数据集或序列中动态物体普遍较少，难以同等强度地检验“定位帮助跟踪、跟踪反哺定位”的双向闭环。本报告并非追求全数据集的榜单排名，而是分析该系统在那些日志、图表及轨迹文件均可直接复现和检查的序列上的表现。

这一立场引出了一个重要的定位说明：\textbf{本报告并非旨在声称达到了 SOTA 性能}。它记录的是一个验证了双向定位-跟踪闭环可行性、展示了完整实验管线并使失效案例透明化的工程原型。对于课程项目而言，这种集成质量、工作流内的可重复运行性及诊断能力本身就是极具价值的成果。一个端到端运行、可多序列回放并能准确暴露失效原因的系统，比一个未经充分调试、仅在表面上显得“紧耦合”的方案更能为后续研究打下坚实基础。
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{p{1.9cm} p{2.5cm} p{2.2cm} p{2.5cm} X}
        ...
        DynaSLAM & 视觉; ORB-SLAM2 家族 & 侧重过滤, 恢复静态图 & 动态检测加背景修复; 较慢但精确~\cite{dynaslam} & 阐明了将动态目标主要视为待移除区域的优势与局限。 \\
...
Conf SLAMMOT 特别值得关注，因为它强调了优化过程内部的置信度引导关联~\cite{confslammot}。这对本项目很有意义，因为目前的后端仍使用相对简单的权重（主要基于检测得分和轨迹长度）。Conf SLAMMOT 传达的底层信息是：置信度不应仅仅是保留或丢弃对象的阈值，它应该影响对应因子的强度。这正是本项目后端如果要提高在不稳定序列上的鲁棒性所需演进的方向。
...
从这些工作中可以归纳出三个宏观观察。首先，\textbf{该领域正朝着更紧密的耦合方向发展}：最强大的近期系统不再保持 SLAM 和 MOT 的完全分离。其次，\textbf{动态物体处理仍主要集中在过滤、关联和对象状态估计}；即使是联合系统也并不总是能将动态物体作为自车定位的几何支撑。第三，\textbf{实时性与工程复杂度依然是决定性因素}，因为随着耦合程度增加，同步、置信度建模与稳定优化变得更加困难。
...
在线模式下共比较三种配置：\texttt{Baseline}、\texttt{FAST-LIO+MCTrack} 和 \texttt{Joint Backend}。\texttt{Baseline} 指纯 FAST-LIO。\texttt{FAST-LIO+MCTrack} 在此基础上增加位姿辅助跟踪与跟踪驱动的动态降权，但直接评估前端优化后的轨迹。\texttt{Joint Backend} 则进一步评估记录自 \texttt{/joint\_backend/odom} 的仅自车后端输出。这样的排列顺序在分析上非常重要，因为它把“前端闭环带来的增益”和“后端额外带来的增益或损失”清晰分离开了。
...
\subsection{从现象到解释}
将上述分组结果综合起来，可以得到一个清晰的因果解释。当基线主要受到动态目标污染前端匹配的影响时，当前系统会稳定获益，因为动态降权正是在配准和建图误差积累之前打断了这一机制。这就是为什么前端闭环改善了全部 7 个所选序列，并且在高漂移动态场景中收益最大。相反，当基线已经相对稳定时，后端的剩余改善空间变小，而通过被跟踪对象施加的额外约束却更容易把噪声重新注入自车位姿。
...
因此，本报告的核心实验结论在当前实际测试范围内是明确的。在这 7 个所选在线序列和当前归档工作流下，项目已经验证了一个有用的前端定位-跟踪闭环，而且其收益可以被持续观察和追溯。这个闭环正是当前系统中最稳定的收益来源。后端方向是有前景的，但仍然不成熟；它的失败并非偶然噪声，而是对下一阶段设计有诊断价值的证据。对于课程项目而言，这已经是一个很强的结果：系统不仅展示了“确实存在改善”，还清楚地区分了改善在哪里是稳健的，在哪里是脆弱的，以及这种差异为什么会出现。

\section{结论与未来计划}
本报告在保持结论与仓库实际状态一致的前提下，把 ME5400 项目扩展成了一套更完整的技术叙述。这个项目已经完成了一件真正有意义的事：它建立了 FAST-LIO、PointPillars 和 MCTrack 之间的完整双向闭环；它通过明确的 ROS 话题和项目定义消息将这个闭环暴露出来；它提供了在当前仓库工作流内可脚本化重跑的实验流程；并且对 7 个所选 KITTI Tracking 在线序列上的轨迹结果进行了定量评估。从这个意义上说，它已经不再只是一个组件综述或一次性演示，而是一个真正运作的工程验证平台，用于研究动态驾驶场景下定位与跟踪如何相互增益。

当前最重要、也最明确验证过的结论其实很简单。系统中最稳定、最可靠的增益，来自“FAST-LIO 位姿辅助 MCTrack”与“MCTrack 将动态权重反馈给 FAST-LIO”构成的前端闭环。这个前端配置在全部 7 个已报告的在线序列上都改善了 ATE RMSE，并在大多数序列上改善了 KITTI 平移误差。因此，动态权重模块并不是某种附属点缀，而是当前系统中最成熟的技术贡献。它以保守、鲁棒且适合在线运行的方式，改善了扫描匹配和局部地图一致性。

本报告同样清楚地说明了哪些问题\emph{还没有}被解决。当前联合后端并不是一个鲁棒的对象中心化优化器。它只是一个在简化假设下测试可行想法的仅自车滑窗原型。它在漂移主导的序列上有时能起作用，但当对象速度外推、时间对齐或门控条件不够可靠时，也可能出现严重退化。Sequence 0009 就是最关键的提醒：把被跟踪对象当作正向定位约束来复用，要比仅仅把它们用于前端软抑制困难得多。这并不否定项目假设，恰恰相反，它正是将该假设精炼为下一阶段更强设计所需的证据。

因此，未来工作的优先级也很清晰。\textbf{第一}，后端应从仅自车修正器演化为一个\textbf{对象中心化联合后端}，在其中对象状态与自车位姿一起被优化，而不是只作为外部观测输入。这是最重要的升级，因为它直接回应了当前系统无法在优化内部吸收对象状态误差的问题。\textbf{第二}，系统需要\textbf{更强的置信度建模与关联策略}。检测得分和轨迹长度是有用的第一步启发，但不足以决定对象因子何时应当强烈影响自车运动。面向置信度的加权、更显式的运动一致性检查以及更丰富的数据关联诊断都仍然需要。\textbf{第三}，优化过程应朝着\textbf{增量式或更高效的滑窗更新}演化。如果对象中心化形式变得更丰富，当前这种轻量级最小二乘循环就不再足够。\textbf{第四}，项目需要在\textbf{更多数据集和更多样的场景}上继续验证，不是为了机械地增加篇幅，而是为了检验当前“前端稳定、后端脆弱”的模式是否会在 KITTI 之外依旧成立。

从课程项目价值的角度看，这是一个强且诚实的结果。该项目已经把多个复杂模块集成进统一工作流，证明了定位-跟踪反馈能够稳定地产生前端收益，也明确暴露了通往更强联合后端所面临的真实工程缺口。一个能工作的系统、能在当前仓库工作流内重跑的实验，以及透明的失败分析，正是让当前原型对后续研究依然有价值的原因。如果下一轮迭代能够把今天这种消息级反馈提升为真正的对象中心化估计图，那么这个项目就能自然地从课程规模的工程验证，继续演化为动态场景 LiDAR-惯导 SLAMMOT 方向上更实质性的研究贡献。
