...
\section{引言}
在动态道路场景中，激光 SLAM 高度依赖于“所观测到的几何特征是静态”这一假设；然而，自动驾驶感知系统最关注的动态对象（如车辆、行人等）恰恰违背了这一前提，导致传统的 SLAM 系统出现地图污染、配准退化以及漂移累积等问题。与此同时，多目标跟踪（MOT）也强烈依赖于 SLAM 提供的可靠自运动补偿（ego-motion compensation）以维持连贯的速度估计与轨迹。因此，两者存在本质上的双向依赖关系：更好的定位能改善跟踪，而更好的跟踪也能反向辅助定位。

本项目正是植根于这一双向直觉。系统采用 FAST-LIO~\cite{fastlio2} 作为定位主干，PointPillars~\cite{pointpillars} 提取 3D 检测，并利用 MCTrack~\cite{mctrack} 生成在线轨迹状态。这些跟踪目标随后通过两种方式被利用：一是转化为软动态权重（soft down-weighting），在 FAST-LIO 前端中削弱动态点带来的误导；二是送入仅含自车位姿（ego-only）的联合后端，测试稳定的目标观测能否提供额外的滑窗约束。为平衡项目周期与工程鲁棒性，系统采用了软降权和松耦合消息闭环方案，而非强依赖紧耦合的完整对象图优化。

据此，本项目追求四个核心目标：\textbf{研究目标}为验证定位与跟踪之间的双向反馈可行性；\textbf{工程目标}为构建可重复运行且组件显式的标准 ROS 管线；\textbf{评估目标}为确定闭环中能够带来稳定增益的环节；\textbf{诊断目标}则为识别系统在联合优化方向上的失效模式。

具体而言，本报告及实现的贡献可总结如下：
\begin{itemize}[leftmargin=2em]
    \item 实现了 FAST-LIO、PointPillars、MCTrack 与 ego-only 后端的完整 ROS 集成，采用项目定义的标准消息接口。
    \item 构建了前端闭环：FAST-LIO 位姿稳定了 MCTrack，而 MCTrack 的输出在 FAST-LIO 预处理中转化为点级软降权。
    \item 建立了脚本驱动的实验框架，支持基线与优化运行的一键切换、\texttt{Results/} 下的标准归档与统一的定量评估。
    \item 通过失效感知分析表明，前端闭环是最稳定的改进源，而当前的后端在速度外推及时间不确定性面前仍显脆弱。
\end{itemize}

选择 KITTI Tracking 作为验证数据集，正是为了利用其丰富的动态参与者（如使用的序列 0003, 0006, 0007, 0009, 0010, 0013, 0020），以充分显现动态点抑制与后端的真实效果~\cite{kitti}。\textbf{本报告并非旨在声称达到了 SOTA 性能}，而是记录了一个验证双向闭环可行性、展示完整实验管线并使失效案例透明化的工程原型，为后续研究打下坚实基础。
